...
export default function AdminCotizaciones() {
  ...
  async function load() {
    ...
    if (isAdmin) {
      const { data: v } = await supabase.from('profiles').select('id, nombre').eq('rol', 'vendedor')
      setVendedores(v || [])
    }
    setLoading(false)
  }
  ...
  const filtered = cotizaciones.filter(c => {
    ...
    if (filtroProvincia && c.provincia !== filtroProvincia) return false
  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#8896a7', fontSize: '14px' }}>{filtered.length} cotizaciones{filtroVendedor || filtroProvincia || search ? ' (filtradas)' : ''}</p>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e6ec', padding: '16px 20px', marginBottom: '20px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        {(search || filtroVendedor || filtroProvincia || filtroFechaDesde || filtroFechaHasta) && (
          <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(''); setFiltroVendedor(''); setFiltroProvincia(''); setFiltroFechaDesde(''); setFiltroFechaHasta('') }} style={{ alignSelf: 'flex-end' }}>
            Limpiar
          </button>
        )}
      </div>
            ...
            <div style={{ padding: '16px 20px' }}>
              ...
              {[
                ...
                <div key={k} style={{ marginBottom: '10px' }}>
                  ...
                  <div style={{ fontSize: '14px', fontWeight: '500', textTransform: 'capitalize' }}>{v}</div>
                </div>
              ))}
